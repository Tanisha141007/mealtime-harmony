...
import { RECIPES, SLOT_LABEL, SLOT_ORDER, type MealSlot, type Recipe } from "@/lib/recipes";
...
function Discover() {
  const { prefs, week, assign, today } = usePlanner();
  const [picked, setPicked] = useState<Recipe | null>(null);

  const region = prefs.location.split(", ")[1] ?? "Kerala";

  const rows: { title: string; items: Recipe[] }[] = [
    { title: "Seasonal picks", items: RECIPES.filter((r) => r.season === "monsoon").slice(0, 8) },
    { title: "High protein", items: RECIPES.filter((r) => r.category === "protein").slice(0, 8) },
    { title: "Quick to cook", items: RECIPES.filter((r) => r.minutes <= 20).slice(0, 8) },
    { title: `Popular in ${region}`, items: RECIPES.filter((r) => r.region === region).slice(0, 8) },
    { title: "Carb-rich comfort", items: RECIPES.filter((r) => r.category === "carb").slice(0, 8) },
    { title: "Veg-forward", items: RECIPES.filter((r) => r.category === "vegetable").slice(0, 8) },
  ];

  const addTo = (slot: MealSlot, date: string) => {
    if (!picked) return;
    assign(date, slot, picked.id);
          ...
          <div className="space-y-4 px-4 pb-8">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {SLOT_ORDER.filter((s) => picked?.slots.includes(s)).map((slot) => (
                <button
                  key={`t-${slot}`}
                  onClick={() => addTo(slot, week[1]!.date)}
